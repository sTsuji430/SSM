...
**【重要：データ分析時の注意点（ダミー試行の除外）】**
本課題では、SVOの選択試行（`task: 'svo'`）だけでなく、その前後に表示される「注視点（空白画面）」や「選択後のフィードバック画面（ハイライト）」もjsPsychの仕様上、データとして記録されます。
分析時には、本命の選択データだけを抽出するため、出力されたデータから `task` カラムが `"svo"` となっている行だけをフィルタリングして使用してください。
※空白画面は `task: 'svo_blank_dummy'`、フィードバック画面は `task: 'svo_feedback_dummy'` として記録されます。
